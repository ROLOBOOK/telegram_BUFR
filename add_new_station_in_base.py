...
indexs_stations = [i[0] for i in get_from_table('numberStation', 'tsao.Stations')]
#получаем список УГМС из базы
list_ugms = [i[0] for i in get_from_table('UGMS','tsao.UGMS')] 
try:
...
list_new_ugms = [(i,) for i in data if i not in list_ugms] 
if list_new_ugms:
...
stations = [(i, indexs[i][0],ugms[indexs[i][1]]) for i in indexs if int(i) not in indexs_stations] 
if stations:
