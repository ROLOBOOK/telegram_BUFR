    ...
    for file_name in files:
        ...
        for telegram in list_telegrams_in_bufr:
            ...
            for telegram in list_telegrams_in_bufr:
                ...
                if meta_inf not in dubl_bufr: dubl_bufr[meta_inf]=list()
                dubl_bufr[meta_inf].append(file_name)
...
if __name__ == '__main__':
    if len(sys.argv) == 2 and sys.argv[1].isdigit():
        ...
        if answer in ('y','yes','да','д','ok'):
            print('start')
            begin = time.time()
            dubl_bufr = main(days=sterday.days)
            r = ''
            for  kye,valum in dubl_bufr.items():
                s = ','.join([i.split('/')[-1] for i in valum[:50]])
                r += f'станция, срок {kye}; количество повторов -  {len(valum)}, в файлах (не больше 50 штук): {s}\n\n'
            t = time.time()-begin
            with open(f'{datenow}_дубли','a') as f:
                f.write(r)
