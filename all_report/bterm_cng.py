...
#'''записываем в словарь даные {индекс_cтанции: {день : данные}} '''
    index_date_dict = {i:[] for i in index_name_cng.keys()}
...
dic_index_ugms = {data.split()[0]:data.split()[-1] for data in index_cng}
list_temp = ['027', '128','129','053', '058', '160','162','068','069','075', '076', '088', '089','03','119','173']
...
ugms_with_sum = {ugms:{i:{i:0 for i in list_temp} for i in list_tetm_2} for ugms in [ug.split()[-1] for ug in index_cng]}
...
with open('/home/bufr/bufr_work/telegram_BUFR/for_work/name_ugms_cng.txt', 'r') as f:
    text = f.read()
...
#11\tПропуски темпертаруты\n14\tПропал сигнал\n15\tСрыв сопровождения\n17\tДлительные пропуски или недостоверные данные\n
    table += '''\n\n\n1 - Разрыв оболочки шара-зонда\n\n2 - Вынужденное снижение шара-зонда в результате обледенения\n\n
3 - Утечка газа или уравновешенный шар-зонд\n\n4 - Слабый или затухающий сигнал\n\n5 - Отказ батареи\n\n6 - Отказ наземного оборудования\n\n
7 - Радиопомехи\n\n8 - Отказ радиозонда\n\n9 - Чрезмерное количество отсутствующих групп данных\n\n10 - Зарезервирована\n\n
11 - Чрезмерное количество отсутствующих данных о температуре\n\n12 - Чрезмерное количество отсутствующих данных о давлении\n\n13 - Прекращение работы пользователя\n\n
14 - Внезапная потеря сигнала\n\n15 - Потеря слежения\n\n16 - Растущее давление\n\n17 - Превышенные предельные сроки недействительных и/или пропущенных данных\n\n30 - Другие'''
