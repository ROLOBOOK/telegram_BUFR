...
cursor = conn.cursor()
cursor.execute(f'''select Stations_numberStation, time_srok, descriptor_035035 from cao_bufr_v2.releaseZonde where year(time_srok)={now.year} and month(time_srok)={month_now} and hour(time_srok)=00 order by day(time_srok);''')
...
with open(f'/home/bufr/reports/{dir_month_now}/{now.year}{now.month:02d}{now.year}{now.month}.bmend', 'w') as f:
    f.write(table)
