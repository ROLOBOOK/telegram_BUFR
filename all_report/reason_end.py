...
for ugms,indexs in sorted(ugm_cng.items()):
    ...
    for index in sorted(indexs):
    # пишем данные о сроках 00
        string_time_00 = ''.join([f"{index_date_00_dict_cng[index][day]:>2}|" if index_date_00_dict_cng[index].get(day, 0)  else "__|" for day in range(1,len_month+1)])
        table += f'{index_name_cng[index]:>21} 00: |{string_time_00}\n'
        # записываем данные о сроках 12
        string_time_12 = ''.join([f"{index_date_12_dict_cng[index][day]:>2}|" if index_date_12_dict_cng[index].get(day, 0)  else "__|" for day in range(1,len_month+1)])
        table += f'{index}{" ":17}12: |{string_time_12}\n'
